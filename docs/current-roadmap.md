...
> **2026-09-09 amendment (7번째)**: §21.2 항목 중 3번(아동 정책)·4번(건강정보 앱 심사
> 대응) 조사 완료, 둘 다 CLOSED. 2번(`/privacy` 라우트화)은 Claude Code 진행 중.
>
> **3. 아동 정책(Families Policy) — 해당 없음**
> - Families Policy는 Play Console "Target audience and content"에서 개발자가
>   **타겟 오디언스에 아동을 포함**으로 선언한 앱에만 적용됨. 우리 앱의 실사용자는
>   보호자(성인)이고 영아 본인이 조작하는 앱이 아니므로 타겟 연령은 성인(18+)으로
>   정직하게 선언하면 됨 → Families Policy 요건(인증 광고 SDK, 개인화 광고 금지,
>   neutral age screen 등) 자체가 적용 안 됨. 어차피 광고/분석 SDK 없음(§21.1 기존
>   확인)이라 해당 지점도 없음.
> - 유일한 주의점: 스토어 리스팅 문구/이미지를 "아이 대상"처럼 홍보하면 실제 타겟
>   선언과 불일치로 재분류될 수 있음(대표적 정책 위반 사례) → 설명문은 "보호자를 위한
>   도구" 톤 유지로 충분, 별도 조치 불필요.
>
> **4. 건강정보 앱 심사 대응 — 액션 없음(당초 판단 정정)**
> - 최초 조사에서 "의료기기 아님" 면책 문구를 설명문에 넣자고 판단했으나, 재검토 결과
>   과잉대응으로 정정. Google의 "의료기기 아님" 문구 의무는 **Health apps declaration에서
>   건강 기능 있음으로 선언한 앱에만** 적용됨.
> - 우리 앱은 진단·치료·건강상태 추적·건강데이터 연동을 하지 않으므로(재료+월령 기반
>   레시피/조리 가이드 제공만 함) Health apps declaration에 **"건강 기능 없음"으로
>   정직하게 선언 가능**. 알레르기 표시 수준은 일반 레시피 앱도 흔히 포함하는 정보라
>   Health & Fitness/Medical 카테고리로 분류되지 않음.
> - 오히려 "의료기기가 아니며..." 문구를 먼저 넣으면 심사자가 Health 앱으로
>   오분류할 리스크를 키움 → 설명문에 넣지 않기로 결정.
> - 참고: 2026-01-28부터 Medical/Health & Fitness 카테고리 앱은 Individual 계정이 아닌
>   Organization 계정이 필요(D-U-N-S 번호 등). 카테고리를 Food & Drink/Parenting으로
>   유지하는 한 해당 없음 — 향후 카테고리 변경 시 재검토 필요.
> - **결정 사항**: 카테고리 Food & Drink/Parenting 유지, Health apps declaration
>   "건강 기능 없음" 선언, 설명문에 의료기기 면책 문구 불포함.
>
...
> ## 2026-09-12 업데이트 — v1.0 Play Store 제출 완료
>
> ### 완료된 것
> - 버그 수정: 쿠킹모드 이미지 누적(key 충돌), 좌측 상단 뒤로가기
>   오동작, TWA 구버전 캐시 고착(force-dynamic), StepInfoTable
>   줄바꿈 레이아웃
> - UX: 알러지 라벨 축약, 다중재료 이미지 그리드, 손질단계 이미지
>   노출 조건 개선, 단계별 형태 추천 문구(reference-only),
>   로딩 화면 신설(LoadingState 공통 컴포넌트 + 일러스트 +
>   점 애니메이션)
> - 데이터: ingredients.beef name_ko "소고기"→"쇠고기" 통일
>   (migration 0063), ingredient_tips 25건 중 20건 문구 축약
>   (migration 0065)
> - 성능: 재료 이미지 217개 리사이즈/재압축(325.5MB→46.8MB),
>   getRecipeLookupData Promise.all 병렬화
> - 앱 이름 "이유식 레시피" → "오늘의 이유식" 변경, 아이콘
>   여백 축소·확대, TWA 재빌드 2회(최종 versionCode 4)
> - QA 체크리스트 28개 전부 종결
> - Play Store 자산 전체 완료: 카테고리(Parenting), 설명문,
>   스크린샷 6장, 피처 그래픽, 개인정보처리방침, Families
>   Policy/데이터 안전/AI 애셋 선언 등 콘솔 설문 완료
> - Google 앱 검토 제출 완료, 비공개 테스트 트랙 설정
>   (12명/14일 요건 대기 중)
>
> ### 남은 백로그 (v1.1 후보)
> 1. 재료 선택 화면 "선택 단계≠추천 단계" mismatch 알림 소실
>    (Phase 11 개편 때 ⭐ 아이콘으로만 대체된 것, 재도입 검토)
> 2. 알레르기 경고 문구 — 버튼 아래 작은 텍스트 노출 + 영어
>    알레르겐 코드(예: (beef)) 그대로 노출, 배너 형태로 격상 필요
>
> ### v2 후보 (기존 §16 유지, 변경 없음)
> 레시피 저장/즐겨찾기, 먹어본 재료 기록, 장보기 리스트,
> 주간/월간 식단, 냉장고 기반 추천, 알레르기/반응 기록,
> 가족 식단, 조리 타이머, 푸시 알림. 광고 도입 시 Families
> Policy 재검토 필수.
>
> ### 중요 원칙 (테스트 기간 중)
> 비공개 테스트 14일 카운트 진행 중에는 프로덕션 트랙에 새
> 빌드를 올리지 않는다 — 테스터 참여 상태 리셋 위험. v1.1/v2
> 개발은 이 기간 동안 로컬/별도 브랜치에서 준비만 하고, 실제
> 배포는 비공개 테스트 완료 후 진행한다.

---
## v1.1 / v1.2 로드맵 (2026-09-12 확정, 벤치마킹 기반)

### 배경
경쟁 벤치마킹(맘마플래너, 냠냠, Solid Starts 등) 결과, "레시피/조리가이드 + 큐브재고 + 캘린더"를 동시에 갖춘 앱이 시장에 없음을 확인. 이 통합 포지션을 v1.1~1.2 목표로 설정.

### v1.1 (비공개 테스트 완료 후 착수, versionCode 5 목표)
1. **큐브 재고관리 MVP** — 단일 재료 큐브 + 복합 큐브(레시피 결과를 하나의 배치로 저장, 예: 소고기죽=소고기+쌀+육수), 식단기록 시 자동 차감, 소진 D-day. 저장소는 클라이언트 IndexedDB(로컬, 무계정 원칙 유지 — DB migration 불필요)
2. **캘린더 다이어리** — 월/주 뷰 전환, 끼니 슬롯 커스텀(2/3/4끼+간식), 신규 도입 재료 색상 표시
3. **알레르기 반응 기록 + 도입 스케줄 자동화** — 반응 발생 시 캘린더에서 즉시 메모, 알레르겐 단계별 도입 스케줄 안내
4. 기존 백로그(§12) 2건 — 재료선택 mismatch 알림 복원, 알레르기 경고 배너화+영어코드 노출 제거

### v1.2
5. **소진 D-day ↔ 레시피 추천 연동** — 임박 큐브 우선 소비 레시피 추천. v1.1의 큐브/캘린더 데이터가 실사용 검증된 뒤 착수 (의존성 있어 순서상 분리)

### 별도 트랙 (배포 일정과 무관하게 진행, 투입 시점 별도 논의)
6. **조리 단계별 사진 확대** — 현재 raw/doneness만 있는 것을 손질/조리 중간 단계까지 확대. 콘텐츠 제작 비용이 커서 릴리즈 일정에 종속시키지 않고 인기 레시피부터 점진 반영. 언제 어느 릴리즈에 포함할지는 추후 디스커션에서 결정.

### 원칙
- 1/2/4는 서로 독립적 — 병렬 개발/개별 검수 가능
...
> **2026-09-13 종료 확인**: 이번 라운드(13afbe7 → af11603 → b67f7d5 → dd052e6
> → 3429aa3) 전 커밋을 Claude Desktop이 git diff/git show로 독립 검수 완료.
> - 코드 변경(13afbe7/af11603): CookingMode 전 STEP 이미지 노출 확장, diff 내용
>   보고와 일치 확인
> - 이미지 자산(b67f7d5): 78장(form 72 + action 6, shrimp 2장 포함) 정확한
>   경로/파일명으로 배치, 코드 변경 0줄 확인
> - main 병합(dd052e6까지 fast-forward): vitest 257/257, tsc 통과 확인
> - 문서 순서 오류(§22가 §21.2 리스트 중간에 삽입됐던 것) 3429aa3에서 교정,
>   내용 변경 없이 위치만 수정된 것 확인
> - main이 origin과 완전 동기화 상태(3429aa3) 확인
>
> 이 라운드는 CLOSED. 다음 세션 우선순위는 인수인계 문서(v2) §18 기준
> 비공개 테스트 트랙 진행 상황(테스터 수, 14일 카운트) 확인.

## 24. 경쟁앱 벤치마킹 기반 기능 후보 (2026-09-14, 논의 대기 — 자동 편입 아님)

> Claude Desktop이 경쟁 앱 조사(국내외 이유식 앱, 특히 "이유식 레시피: 아기
> 식단 관리" 등) 결과 도출한 기능 공백 목록. **결정된 로드맵이 아니라 논의용
> 후보 목록이다** — 아래 항목은 필요성/구현범위/안전성/데이터필요성을 각각
> 논의해 채택 여부를 정한 뒤에만 v1.2 이후 실제 로드맵(§v1.1/v1.2 섹션)으로
> 옮긴다. 지금 시점에 착수 확정된 것은 하나도 없음.

| 후보 | 내용 | 비고 |
|---|---|---|
| 장보기 리스트 | 레시피/식단에 포함된 재료를 장보기 목록으로 뽑아주는 기능 | 인수인계 문서 §16 v2 후보 목록에 이미 있던 항목과 동일 — 신규 아님, 재확인 |
| 하루 자동 식단 플래너 | 아기 월령/식사 유형에 맞춰 하루 단위로 식단을 자동 구성해주는 기능 | 인수인계 문서 §16의 "주간/월간 식단"과 유사 범위 — 통합 논의 필요 |
| 의사 제출용 건강 리포트(PDF) | 반응 기록·신규 도입 재료·식사 기록을 PDF로 묶어 소아과 방문 시 제출 | 신규 후보. 알레르기 반응기록(이미 구현됨) 데이터를 재가공하는 형태라 신규 데이터 모델은 불필요할 가능성 높음(확인 필요) |
| 데이터 백업/복원 | 무계정 로컬 저장 구조의 약점(기기 변경 시 데이터 소실) 보완 | 신규 후보. 무계정 원칙과 충돌하지 않는 방식(로컬 파일 export/import 등)인지 안전성/구현범위 논의 필요 |
| 다자녀(복수 프로필) | 아기 프로필을 여러 개 등록해 전환 사용 | 신규 후보. 현재 데이터 모델(BabyProfile 단일 인스턴스 가정)에 미치는 영향 범위 확인 필요 |

**원칙**: 이 5개는 "논의해서 필요하면 편입"이며, 지금 백로그에 강제로 얹지 않는다.
채택된 항목만 §v1.1/v1.2 섹션의 NOW/NEXT/LATER 구조로 옮겨 정식 편입한다.

### 24-1. 추가 후보: 다크모드 토글 (2026-09-14 추가)

폰 OS 다크모드 시 앱이 검게 깨지는 버그(원인: `color-scheme` 미선언으로 브라우저/
WebView가 강제 다크 반전 적용)를 고치는 김에 논의된 후보. **버그 수정 자체는
이번 세션에서 즉시 진행**(라이트 고정) — 아래는 그와 별개로 "설정에서 다크모드를
직접 켜고 끄는 기능"을 넣을지에 대한 논의 대기 항목.

- 보류 사유: 현재 색상 체계가 전부 라이트 전용 CSS 변수로만 설계돼 있어 실제
  다크테마를 만들려면 전 화면(홈/캘린더/큐브/설정/레시피/쿠킹모드 등) 재검수가
  필요한 규모 있는 작업. 아직 테스터 피드백 등 실사용자 필요성 신호 없음(MVP
  원칙 §5-1 "필요성 확인 전까지 범위 확대 안 함"에 해당).
- 재개 조건: 비공개 테스트/출시 후 다크모드 요청이 실제로 들어올 때 재검토.

### 24-2. TWA 강제 다크모드 미해결 — 네이티브 빌드 필요 (2026-09-14 추가)

**증상**: Play Store로 설치한 TWA 앱에서, 폰 OS 다크모드 시 화면이 검게
반전됨. 웹 레벨 대응 2단계 전부 시도했으나 미해결:
1. `color-scheme: only light` (globals.css) — 미해결
2. `@media (prefers-color-scheme: dark)` 내 라이트 값 재선언 — 미해결

**결론**: 삼성 인터넷 계열 WebView의 구버전 강제 다크(Force Dark) 휴리스틱이
웹 표준 `color-scheme` 신호 자체를 무시하는 것으로 추정. 웹 코드로는 더 이상
대응 불가 — 남은 방법은 TWA 네이티브 빌드 레벨(`twa-build/`)에서
`android:forceDarkAllowed="false"`를 지정하는 것뿐(현재 커스텀 styles.xml이
없어 시스템 테마 `Theme.Translucent.NoTitleBar`를 그대로 씀 — 커스텀 테마
신설 필요).

**지금 하지 않는 이유**:
- 네이티브 빌드 변경 → 재빌드 → 신규 versionCode 필요한 작업인데, 현재
  비공개 테스트 14일 카운트 진행 중이라 §19 원칙상 새 빌드 업로드 자체가
  금지된 시점.
- 효과가 100% 보장되지도 않음(구버전 WebView는 이마저도 무시할 가능성 있음).

**재개 조건**: 비공개 테스트 완료 후, 다음 버전 빌드 시점에 함께 진행.

## v1.1 진행상황 정리 (2026-09-13, Claude Desktop 세션 종료 시점)

### main(f6d6183) 반영 완료 항목
1. **큐브 재고관리** (feat 3132d06) — 단일/복합 큐브, IndexedDB, 레시피 결과 화면 "큐브로 저장" 연동, 식단기록 시 자동차감 함수까지 구현(UI 연동은 알레르기 체크인 작업에서 다이어리 연동으로 대체됨)
2. **캘린더 다이어리** (feat 9f8c46c) — 월/주 뷰 토글, DiaryEntry 로컬 저장, 신규재료 색상표시
3. **알레르기 반응기록** (feat 6ffd61c) — AllergenIntroduction 데이터모델, 도입 스케줄 정적 안내
4. **알레르기 경고 UX 배너화 + 영어코드 제거** (백로그 §12-2, fix 4e7834d)
5. **재료선택 mismatch 안내 문구 복원** (백로그 §12-1, fix 6e8b6aa) — Phase 10 원본 문구 그대로 복원 확인됨
6. **전역 하단 네비게이션 바** (feat ba252df) — 홈/캘린더/레시피(중앙강조)/큐브재고/설정 5탭, 레시피 탭은 입력→결과→조리모드 3화면 소속 처리, 조리모드 진행상태 로컬 영속화(24시간 내 복원)
7. **알레르기 체크인 모달** (feat f6d6183) — 홈 진입 시 "어제 새로 먹인 재료" 이상반응 확인 팝업, 1일차 1차확인→3일 경과 시 확인완료 전환, 7일 경과 시 재질문 중단, 설정 화면에 확인완료/의심재료 목록 노출
8. **조리 단계별 사진 확대** — 별도 대화창에서 완료(stage-form 이미지 78장 추가, 커밋 b67f7d5 등). 상세 범위/재료 커버리지는 해당 세션 기록 참고

### 별도 확인된 사항
- 딸기/블루베리/씨앗제거 오류 수정 + 귤 정책 override (fix 62a8544) — 다른 세션에서 승인·실행됨, 이번 라운드에 main 정리하며 함께 반영됨

### 남은 v1.2 항목
- D-day↔레시피 추천 연동 — 1/2번 실사용자 데이터 검증 후 착수 예정 (비공개 테스트 재개 대기 중)
