...
## Claude Desktop ↔ Claude Code 협업 워크플로우

이 프로젝트는 Claude Desktop(웹, PM 역할)과 Claude Code(로컬, 구현 역할) 두 세션이 함께 작업한다. 아래 규칙은 두 세션 간 결과 전달 방식에 대한 것이며, 모든 Claude Code 세션이 따라야 한다.

### 1. Claude Code → Claude Desktop 결과 전달

- 채팅 복사/붙여넣기로 전달하지 않는다.
- 결과는 `docs/claude-desktop-handoff/` 디렉터리 아래 마크다운 파일로 작성한 뒤 commit + push 한다.
- 파일명 규칙: `YYYY-MM-DD-{주제}.md` (예: `2026-08-30-backlog-review.md`)
- 이 디렉터리에 새 마크다운 파일을 추가/커밋/push 하는 것은 매번 사용자 승인 없이 진행할 수 있다.
  - 단, DB 변경 / migration 실행 / 코드 수정 / 기존 파일 삭제는 여전히 별도 승인이 필요하다 (변경 없음).

### 2. 보고서 작성 원칙

- 이 보고서는 사람이 아니라 Claude Desktop이 읽는다.
- 요약/서론/배경 재설명/"~했습니다" 식 자연어 설명을 최소화하고, 실제 값(숫자·파일 경로·커밋 해시·코드·DB row 값·boolean)을 표/목록/코드블록 등 구조화된 형태로 직접 제시한다.
- 이미 알고 있는 맥락(인수인계 문서 내용 등)을 다시 설명하지 않는다.
- 확인 불가능한 항목은 추측하지 말고 "확인 불가: [이유]"로 명시한다.

### 3. git push 정책

- 이미 존재하는 로컬 커밋을 origin에 반영하는 push는, 안전하다고 판단되면 사용자 승인 없이 진행할 수 있다.
- 새로운 커밋 생성(코드/DB/migration 변경을 포함하는)은 여전히 사용자 승인이 필요하다. push는 "이미 승인된 것을 반영하는 행위"로 별도 취급한다.

### 4. 사용자 채팅 응답 원칙

- 결과 보고서는 위 §1~2 규칙대로 `docs/claude-desktop-handoff/` 마크다운 파일에만 작성한다.
- 이 파일을 commit + push한 뒤, 사용자에게 보내는 채팅 응답에는 보고서 내용을 요약/재서술하지 않는다.
- 완료 시 채팅에는 반드시 3줄 형식을 명시한다: (1) 원격 DB/코드 실제 실행 여부 (2) 로컬 파일 생성·수정 여부 (3) commit/push 여부. 상세 수치·값·근거는 여전히 보고서 파일에만 쓰고 채팅에서 재서술하지 않는다 — 이 3줄은 요약이 아니라 사용자가 다음 행동(승인/커밋 지시 등)을 판단하는 데 필요한 최소 정보다.
- 작업 중 승인이 필요하거나 사용자가 직접 알아야 할 이슈(예: 위험한 작업 확인, 차단 상황)가 있으면 그 내용만 간단히 전달하고, 나머지는 보고서 파일로 대체한다.
