...
interface BabyProfileSettingsViewProps {
  stages: Stage[];
  allergens: Allergen[];
...
/**
 * /settings/profile hosts the profile-edit form that used to open inline over
 * BabyHome via its "아기 정보 수정" menu item (전역 하단바 도입 — 아기 정보
 * 수정은 이제 별도 탭/화면). 이후 설정이 목록 화면(/settings)으로 바뀌면서
 * 이 폼은 "아기 정보" 항목의 하위 경로로 이동했다 — BabyHome의 메뉴는
 * /settings/profile로 직접 연결된다(목록 화면 거치지 않음). BabyProfileGate의
 * `?edit=1` 온보딩 경로(= /plan의 "알레르기 정보 수정" 버튼)는 그대로 유지되며
 * 이 화면과 무관하다.
 */
...
export function BabyProfileSettingsView({ stages, allergens, ingredients }: BabyProfileSettingsViewProps) {
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="mb-2 flex items-center gap-3">
        ...
        <h1 className="font-serif-kr text-xl font-bold tracking-tight text-[var(--ink-900)]">아기 정보</h1>
      </div>
