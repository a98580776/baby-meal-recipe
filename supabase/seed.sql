...
-- =======================================================================
-- Migration 0044 addition (append-only, mirrors that migration's data
-- portion) -- see supabase/migrations/0044_grain_consistency_texture.sql and
-- docs/claude-desktop-handoff/2026-09-01-grain-consistency-policy-design.md
-- for full rationale (rice/oatmeal/brown_rice/barley texture_profiles, 4
-- stage-uniform rows each, per KDCA's "applies across all stages" wording;
-- shape/particle_size stay null -- porridge consistency is not a "cut shape"
-- concept, schema-freeze.md §10-1).
-- =======================================================================
...
-- =======================================================================
-- Migration 0047 addition (append-only, mirrors that migration's data
-- portion) -- see supabase/migrations/0047_c2_remaining_8_prep_fields.sql
-- and docs/c2-remaining-9-investigation.md. C-2 남은 8건 cutting_guidance
-- boilerplate 해소(napa_cabbage/cabbage/onion/radish/green_pea/kidney_bean/
-- sesame/broccoli). perilla는 TIER_1 출처 부재로 이번 migration에서 제외.
-- =======================================================================
...
-- =======================================================================
-- Migration 0049 addition (append-only, mirrors that migration's data
-- portion) -- see supabase/migrations/0049_ingredient_tips_batch2.sql.
-- ingredient_tips 2차 배치: 8개 재료(egg/salmon/pork/onion/kidney_bean/
-- green_pea/chestnut/cheese) 각 2건씩 총 16건 INSERT. 전부 기존에 이미 DB에
-- 존재하는 재료-특정 TIER_1 evidence 재사용, 신규 evidence 없음.
-- =======================================================================
...
-- =======================================================================
-- Migration 0050 addition (append-only, mirrors that migration's data
-- portion) -- see supabase/migrations/0050_ingredient_tips_batch3.sql.
-- ingredient_tips 3차 배치: 8개 재료(cod/tuna/seaweed/sesame/radish/cabbage/
-- napa_cabbage/beef) 각 2건씩 총 16건 INSERT. 15건은 기존에 이미 DB에 존재하는
-- 재료-특정 TIER_1 evidence 재사용(신규 evidence 없음), 1건(tip_seaweed_2)만
-- evidence_id 없이 source_note로 이 프로젝트 자체 데이터를 인용.
-- =======================================================================
...
-- =======================================================================
-- Migration 0053 addition (append-only, mirrors that migration's data
-- portion) -- see supabase/migrations/0053_ingredient_tips_batch6.sql.
-- Part A: strawberry/blueberry/grape tips (기존 E014 재사용, 신규 evidence 없음).
-- Part B: kiwi/tangerine/mango/pear/banana/avocado -- 신규 evidence E056~E061
-- 등록 + preparation_profiles boilerplate REPLACE(UPDATE, 원본 INSERT 문은
-- 무수정) + tips. perilla는 포함하지 않음(batch5 candidates 문서 §9에서 확정 제외).
-- =======================================================================
...
-- =======================================================================
-- Migration 0056 additions (append-only, mirrors that migration's data
-- portion so a fresh bootstrap matches the migrated state -- see that
-- file for full sourcing rationale). 재료 확장 배치1: 50 -> 70개 (신규 20개).
-- Source: docs/claude-desktop-handoff/migration-batch1-review-packet.md
-- =======================================================================
...
-- Migration 0058 addition (append-only, mirrors that migration's data portion,
-- 실행된 원문 그대로 -- evidence 컬럼명은 실제 스키마(url/checked_at/applicability)로
-- 수정해 반영, 값 내용은 draft와 동일) -- 전복(abalone) choking safety 정책.
insert into evidence (id, organization, title, url, source_tier, checked_at, applicability, status) values
...
-- Migration 0059 addition (append-only, mirrors that migration's already-applied
-- remote DB state) -- 런칭 체크리스트 Tier1(공식 알레르기 재료 중 근거 없음) 3종 해소.
...
-- Migration 0060 addition (append-only, mirrors that migration's already-applied
-- remote DB state) -- 새우: 재료 특정 근거 신규(Solid Starts) -- 문어와 동일한
-- 원통형 질식 기전.
...
-- Migration 0061 addition (append-only, mirrors that migration's already-applied
-- remote DB state) -- 가족 영양사 실검토 반영: Tier2 11개 재료(evidence: E018/E074/
-- E016/E065/E064/E087/E083/E084/E085·E013/E016/E072) 2026-09-09 실제 검토 완료.
